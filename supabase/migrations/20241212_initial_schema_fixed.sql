...
-- Leads (CREATE THIS BEFORE CONVERSATIONS)
CREATE TABLE leads (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    bot_id UUID NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
    conversation_id UUID, -- Will add FK constraint after conversations table
...
-- Conversations (NOW CAN REFERENCE LEADS)
...
-- Add FK constraint from leads to conversations (now that both exist)
ALTER TABLE leads
